...
You can include pseudocode or algorithms using the \texttt{algorithm} environment. Here is an example of a disjoint decomposition algorithm:
\begin{algorithm}
...
\subsection{Another Algorithm Example}
Here is another example demonstrating Dijkstra's algorithm for finding the shortest paths in a graph. This example showcases initialization, a main loop, and conditional logic within the algorithm.

\begin{algorithm}[H]
  \caption{Dijkstra's Algorithm for Shortest Paths}
  \label{algo:dijkstra}
  ...
  \Input{A weighted graph $G=(V, E)$ with non-negative weights, and a source vertex $s \in V$}
  \Output{The shortest path distances from $s$ to every other vertex $v \in V$}

  \BlankLine
  \tcp{Initialization for all vertices}
  \ForEach{vertex $v \in V$}{
    \Dist$[v] \leftarrow \infty$\;
    \Prev$[v] \leftarrow \text{undefined}$\;
  }
  \Dist$[s] \leftarrow 0$\;
  \Queue $\leftarrow V$\tcp*{A min-priority queue of vertices, keyed by their \Dist values}

  \BlankLine
  \tcp{Main loop to find shortest paths}
  \While{\Queue is not empty}{
    $u \leftarrow$ \ExtractMin{\Queue}\tcp*{Vertex with smallest distance}
    \ForEach{neighbor $v$ of $u$}{
      \tcp{Relaxation step}
      $alt \leftarrow \Dist[u] + \text{weight}(u, v)$\;
      \If{$alt < \Dist[v]$}{
        \Dist$[v] \leftarrow alt$\;
        \Prev$[v] \leftarrow u$\;
        \DecreaseKey{\Queue, $v$, $alt$}\tcp*{Update priority in the queue}
      }
    }
  }
  \BlankLine
